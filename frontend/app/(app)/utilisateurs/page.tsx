      ...
      <div className="px-8 py-7 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-fraunces)', fontSize: '1.7rem', color: '#0f172a', fontWeight: 500 }}>
              Utilisateurs
            </h1>
          </div>
          <button
        ...
        {formOpen && (
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input required value={firstName} onChange={(e) => setFirstName(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-lg text-sm" style={{ border: '1.5px solid #e2e8f0', color: '#0f172a' }} />
              </div>
              <div>
            ...
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>Email professionnel</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={role} onChange={(e) => setRole(e.target.value as Role)}
                  className="w-full px-3.5 py-2.5 rounded-lg text-sm bg-white" style={{ border: '1.5px solid #e2e8f0', color: '#0f172a' }}>
                  {ROLE_OPTIONS.map((o) => (
              ...
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>Mot de passe provisoire</label>
            ...
            {error && (
              <div className="px-3.5 py-2.5 rounded-lg text-sm" style={{ background: '#fef2f2', color: '#b91c1c', border: '1px solid #fecaca' }}>
                {error}
              </div>
            )}
        ...
        <div className="bg-white rounded-xl2 overflow-hidden" style={{ border: '1px solid #e2e8f0' }}>
          {loading && <p className="px-5 py-4 text-sm" style={{ color: '#94a3b8' }}>Chargement…</p>}
          {!loading && users.length === 0 && (
            <p className="px-5 py-4 text-sm" style={{ color: '#94a3b8' }}>Aucun utilisateur.</p>
          )}
          {!loading && users.map((u, i) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold truncate" style={{ color: u.is_active ? '#0f172a' : '#94a3b8' }}>
                  {u.first_name} {u.last_name}
                  {!u.is_active && <span className="ml-2 text-xs font-normal">(désactivé)</span>}
                </p>
              ...
              <select
                ...
                style={{ border: '1.5px solid #e2e8f0', color: '#1d4ed8' }}
              >
                {ROLE_OPTIONS.map((o) => (
                  <option key={o.value} value={o.value}>{o.label}</option>
                ))}
              ...
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2.5 3.5h9M5.5 3.5V2h3v1.5M5.5 6.5v4M8.5 6.5v4M3.5 3.5l.5 8.5h6l.5-8.5"
        ...
        <p className="text-xs mt-3" style={{ color: '#94a3b8' }}>
          {ROLE_LABELS.associe} : {users.filter((u) => u.role === 'associe').length} · Total : {users.length}
        </p>
      </div>
    </div>
  );
