...
import MissionAccessModal from './MissionAccessModal';
...
export default function MissionsPanel() {
  ...
  const [accessMissionId, setAccessMissionId] =
  ...
  /*
   * Seul un associé peut :
   * - créer une entité
   * - supprimer une entité
   * - créer une mission
   * - modifier les accès d'une mission
   * - supprimer une mission
   */
  const canManage = currentUser?.role === 'associe';
  ...
  const filteredMissions = useMemo(() => {
    ...
    if (dateFilter === 'proche') {
      ...
      list = list.filter(
        (m) =>
          new Date(m.closing_date).getFullYear() ===
          year
      );
    }

    return [...list].sort((a, b) =>
      a.closing_date.localeCompare(b.closing_date)
    );
  }, [
    missionsForActiveEntity,
    search,
  ...
  async function handleDeleteEntity(
    ...
    if (
      !confirm(
        'Supprimer cette entité ? Cela suppose qu’elle n’a plus aucune mission.'
      )
    ) {
      return;
    }
  ...
  async function handleDeleteMission(
    ...
    if (
      !confirm(
        'Supprimer cette mission ? Cette action est irréversible (réponses et documents inclus).'
      )
    ) {
      return;
    }
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2.5 py-2.5 space-y-2">
          ...
          {filteredEntities.map((e) => (
            ...
            >
              <p
                className="text-base font-semibold truncate mb-0.5 pr-6"
                style={{ color: '#0f172a' }}
              >
                {e.sigle || e.name}
              </p>
              ...
              {canManage && (
                ...
                >
                  ...
                  >
                    <path
                      d="M2.5 3.5h8M5.5 3.5V2h2v1.5M5 3.5l.5 8h2l.5-8"
                      stroke="currentColor"
                      strokeWidth="1.2"
                      strokeLinecap="round"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          className="text-base font-bold truncate mb-3 text-center"
          style={{ color: '#0f172a' }}
        >
          {activeEntity.sigle ||
            activeEntity.name}
        ...
        >
          <option value="toutes">
            Toutes les dates de clôture
          </option>

          <option value="proche">
            Clôture dans les 30 jours
          </option>
        ...
        {!loading &&
          filteredMissions.length === 0 && (
            <p
              ...
              style={{ color: '#94a3b8' }}
            >
              Aucune mission pour cette entité.
            </p>
          )}
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-1.5">
                <p
                  className="text-base font-semibold truncate mb-1.5 pr-1"
                  style={{ color: '#0f172a' }}
                >
                  {m.name}
                </p>
                ...
                {canManage && (
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    >
                      ...
                      >
                        <path
                          d="M6.5 1.8a2.2 2.2 0 100 4.4 2.2 2.2 0 000-4.4ZM2.2 11c.2-2.2 1.7-3.4 4.3-3.4s4.1 1.2 4.3 3.4"
                          stroke="currentColor"
                          strokeWidth="1.1"
                          strokeLinecap="round"
                        />
      ...
      >
        Total : {missionsForActiveEntity.length}{' '}
        mission
        {missionsForActiveEntity.length > 1
          ? 's'
          : ''}
      </div>
      ...
      <MissionAccessModal
        open={!!accessMissionId}
        missionId={accessMissionId}
        onClose={() =>
          setAccessMissionId(null)
        }
      />
    </div>
  );
